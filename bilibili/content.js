...
let pendingLearningSave = null;
let obsidianSaveInFlight = null;
...
function logInfo(..._args) {
  if (shouldDebugLog()) {
    console.info("[BOC] debug status");
  }
}

function logWarn(..._args) {
  if (shouldDebugLog()) {
    console.warn("[BOC] debug warning");
  }
}
...
function sendToObsidian() {
  if (obsidianSaveInFlight) return obsidianSaveInFlight;
  const sendButton = document.getElementById(ids.sendBtn);
  if (sendButton) sendButton.disabled = true;
  obsidianSaveInFlight = performObsidianSave().finally(() => {
    obsidianSaveInFlight = null;
    if (sendButton) sendButton.disabled = false;
  });
  return obsidianSaveInFlight;
}

async function performObsidianSave() {
  state.settings = await getSettings();
  await refreshDerivedContent();
  if (!state.markdown) {
  ...
  try {
    ...
    setStatus("正在生成学习资料…");
    const learningDocument = buildBilibiliLearningDocument();
    const saveKey = JSON.stringify({ baseUrl, filepath, learningDocument, originalMarkdown: state.markdown });
    if (pendingLearningSave?.key !== saveKey) {
      pendingLearningSave = null;
      const learningArtifacts = await prepareLearningArtifacts(learningDocument);
      pendingLearningSave = {
        key: saveKey,
        filepath,
        markdown: insertLearningMarkdownBeforeSubtitle(state.markdown, learningArtifacts.markdown),
        canvasPath: buildLearningCanvasPath(filepath),
        canvas: JSON.stringify(learningArtifacts.canvas, null, 2),
        files: { markdown: false, canvas: false }
      };
    }
    const prepared = pendingLearningSave;

    if (!prepared.files.markdown) {
      try {
        setStatus("正在写入 Markdown…");
        await writeNoteByLocalApi(baseUrl, apiKey, prepared.filepath, prepared.markdown, "text/markdown");
        prepared.files.markdown = true;
      } catch (error) {
        setStatus("Markdown 写入失败；Canvas 未写入。");
        setMessage(`Markdown 写入失败；Canvas 未写入：${getErrorMessage(error)}`);
        return;
      }
    }

    if (!prepared.files.canvas) {
      try {
        setStatus("正在写入 Canvas…");
        await writeNoteByLocalApi(
          baseUrl,
          apiKey,
          prepared.canvasPath,
          prepared.canvas,
        ...
        prepared.files.canvas = true;
      } catch (error) {
        setStatus("Markdown 已写入；Canvas 写入失败。");
        setMessage(`Markdown 已写入；Canvas 写入失败：${getErrorMessage(error)}`);
        return;
      }
    }

    setStatus("学习 Markdown 和 Canvas 已写入。");
    setMessage(`已写入 Obsidian：${prepared.filepath}（Markdown + Canvas）`);
    pendingLearningSave = null;
  } catch (error) {
...
async function prepareLearningArtifacts(document = buildBilibiliLearningDocument()) {
  const response = await sendRuntimeMessage({
    type: "learning:prepare",
    payload: { document }
  });
  const prepared = response?.value;
  if (
    !response?.ok ||
    !prepared?.outputs ||
    ...
    !prepared.canvas ||
    typeof prepared.canvas !== "object"
  ) {
    throw new Error(toReadableText(response?.error, "学习资料生成失败，请重试。"));
  }
  return prepared;
}

function buildBilibiliLearningDocument() {
  const subtitleBody = Array.isArray(state.subtitleBody) ? state.subtitleBody : [];
  const withHours = shouldShowHoursInSubtitle(subtitleBody);
  const mainContent = subtitleBody
    .map((item) => {
      const text = String(item?.content || "").trim();
      return text ? `\`${formatCompactTimestamp(item?.from, withHours)}\` ${text}` : "";
    })
    .filter(Boolean)
    .join("\n");
  ...
  return {
    ...
    url: cleanVideoUrl(),
    publishedAt: String(state.uploadDate || "").trim(),
    description: String(state.description || "").trim(),
    mainContent,
    contentCompleteness: {
      transcript: subtitleBody.length > 0,
      description: Boolean(String(state.description || "").trim())
    },
    metadata: {
      bvid: String(state.bvid || ""),
      aid: String(state.aid || ""),
      cid: String(state.cid || ""),
      subtitleLanguage: String(state.selectedSubtitleLang || ""),
      pageIndex: Number(state.pageIndex) || 1,
      pageCount: Number(state.pageCount) || 0
    }
  };
}

function insertLearningMarkdownBeforeSubtitle(markdown, learningMarkdown) {
  const original = String(markdown || "");
  const shared = String(learningMarkdown || "").trim();
  const headingMatch = /(?:^|\r?\n)## 字幕(?:\r?\n|$)/m.exec(original);
  if (!shared || !headingMatch) {
    throw new Error("学习 Markdown 无法插入字幕段落。");
  }
  const headingIndex = headingMatch.index + (headingMatch[0].startsWith("\r\n") ? 2 : headingMatch[0].startsWith("\n") ? 1 : 0);
  return `${original.slice(0, headingIndex)}${shared}\n\n${original.slice(headingIndex)}`;
}

function buildLearningCanvasPath(markdownPath) {
  const path = String(markdownPath || "");
  if (!/\.md$/i.test(path)) {
    throw new Error("Markdown 路径无效，无法生成 Canvas 路径。");
  }
  return `${path.slice(0, -3)}.canvas`;
}
...
async function writeNoteByLocalApi(baseUrl, apiKey, filepath, content, contentType = "text/markdown") {
  const resp = await sendRuntimeMessage({

...
const EXPECTED_CONTENT_SCRIPT_VERSION = "1.1.3";

BOC_LIFECYCLE.runtimeOnInstalled(async () => {
...
BOC_LIFECYCLE.runtimeOnMessage((message, sender, sendResponse) => {
  ...
  if (message.type === "open-options") {
    chrome.tabs
      .create({ url: chrome.runtime.getURL("bilibili/options.html") })
      .then(() => sendResponse({ ok: true }))
      .catch((error) => sendResponse({ ok: false, error: error.message }));
    return true;
  }
  ...
  if (message.type === "write-obsidian-note") {
    ...
    const contentType = String(message.contentType || "text/markdown").trim();
    ...
    if (contentType !== "text/markdown" && contentType !== "application/json") {
      sendResponse({ ok: false, error: "不支持的 Obsidian 内容类型" });
      return false;
    }
...
BOC_LIFECYCLE.runtimeOnConnect((port) => {

...
function loadBackgroundHelpers({
  ...
  const sandbox = {
    ...
    chrome: {
      ...
      runtime: {
        ...
        onMessage: listeners,
        openOptionsPage() {},
        getURL: (resourcePath) => `chrome-extension://test/${resourcePath.replace(/^youtube\//, "")}`,
  ...
  sandbox.globalThis = sandbox;
  vm.runInNewContext(read("lifecycle.js"), sandbox);
  vm.runInNewContext(read("background.js"), sandbox);
  return sandbox.__YTD_TRANSLATION_TESTING__;
}

// Standalone close-on-other-sites tests are replaced by unified routing tests.
...
test("all AI product requests use DeepSeek non-thinking and JSON behavior", async () => {
  ...
  assert.equal(
    ...
    5, // Includes the retained unified learning-output handler.
  );

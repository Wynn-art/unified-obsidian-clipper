...
importScripts("youtube/settings.js");
importScripts("youtube/obsidian.js");
...
async function getObsidianSettings() {
  ...
  return YTD_OBSIDIAN.normalizeSettings(settings.obsidian);
}

async function requestObsidian(method, path, body, contentType = "text/markdown") {
  const settings = await getObsidianSettings();
  if (!settings.apiKey) throw new Error("Obsidian API Key 未配置，请打开 Settings 设置。");
  const url = path === "root" ? `${settings.baseUrl}/` : YTD_OBSIDIAN.vaultUrl(settings.baseUrl, path);
  ...
  if (!response.ok) {
    const error = new Error(YTD_OBSIDIAN.mapObsidianError(response.status));
...
async function handleTestObsidianConnection() {
  await requestObsidian("GET", "root");
  return { success: true };
}

async function handleResolveLearningObsidianTarget(metadata) {
  const settings = await getObsidianSettings();
  const markdownPath = YTD_OBSIDIAN.buildNotePath(settings, metadata);
  return {
    success: true,
    markdownPath,
    canvasPath: YTD_OBSIDIAN.learningCanvasPath(markdownPath),
  };
}
...
async function handleSyncToObsidian(
  ...
  if (artifacts.markdown !== false) {
    try {
      ...
      return { success: false, error: `Markdown 保存失败；Canvas 未保存：${error.message}`, markdownPath, canvasPath, files };
    }
  }
  if (artifacts.canvas !== false) {
    if (!(outputs || learningCanvas)) {
      return { success: false, error: "Markdown 已保存，但 Canvas 保存失败：缺少学习输出。", markdownPath, canvasPath, files };
    }
    try {
      const canvas = learningCanvas || YTD_OBSIDIAN.buildCanvas(outputs);
      await requestObsidian("PUT", canvasPath, JSON.stringify(canvas, null, 2), "text/plain");
      files.canvas = true;
    } catch (error) {
...
async function handleOpenObsidianNote(path) {
  const settings = await getObsidianSettings();
  const response = await fetch(YTD_OBSIDIAN.openUrl(settings.baseUrl, path), {
    method: "POST",
    headers: { Authorization: `Bearer ${settings.apiKey}` },
  });
  if (!response.ok) throw new Error(YTD_OBSIDIAN.mapObsidianError(response.status));
...
async function loadPromptSection(fileName, heading, variables = {}) {
  ...
  if (!markdown) {
    const response = await fetch(chrome.runtime.getURL(`youtube/prompts/${fileName}`));
    if (!response.ok) {
...
async function handleGenerateLearningOutputs({
  transcriptText,
  videoTitle,
  channelName,
  videoDescription,
  ...
  try {
    const promptVariables = {
      transcriptText: String(transcriptText || "").slice(0, 180000),
      videoTitle: videoTitle || "Unknown",
      channelName: channelName || "Unknown",
      videoDescription: String(videoDescription || "No description available").slice(0, 12000),
    };
    const systemPrompt = await loadPromptSection("learning-outputs.md", "System prompt", promptVariables);
    const userPrompt = await loadPromptSection("learning-outputs.md", "User prompt", promptVariables);
    ...
    const outputs = YTD_OBSIDIAN.validateLearningOutputs(parseLooseJson(text));
    return { success: true, outputs };
  } catch (error) {
    return { success: false, error: error.message || "AI 学习结果生成失败。" };
  }
}
...
YTD_LIFECYCLE.tabsOnUpdated((tabId, changeInfo) => {
...
YTD_LIFECYCLE.tabsOnActivated(async ({ tabId }) => {
  try {
 ...
 */
YTD_LIFECYCLE.runtimeOnMessage((message, sender, sendResponse) => {
  ...
  if (message.action === "generateLearningOutputs") {
    handleGenerateLearningOutputs(message)
      .then(sendResponse)
  ...
  if (message.action === "testObsidianConnection") {
  ...
  if (message.action === "resolveLearningObsidianTarget") {
    handleResolveLearningObsidianTarget(message.metadata)
      .then(sendResponse)
      .catch((error) => sendResponse({ success: false, error: error.message }));
    return true;
  }
  ...
  if (message.action === "openObsidianNote") {
    handleOpenObsidianNote(message.path)
      .then(sendResponse)
      .catch((error) => sendResponse({ success: false, error: error.message }));
    return true;
  }
  ...
  if (message.action === "openOptions") {
    chrome.tabs.create({ url: chrome.runtime.getURL("youtube/options.html") });

const YTD_OPTIONS = (() => {
  ...
  const COPY = {
    en: {
      ...
      obsidianProvider: "Obsidian sync",
      obsidianApiUrlLabel: "Obsidian API address",
      obsidianApiKeyLabel: "Obsidian API key",
      obsidianFolderLabel: "Vault folder",
      obsidianFilenameLabel: "Filename template",
      obsidianHelp: "Install and enable the Obsidian Local REST API plugin with its local HTTP server.",
      testObsidian: "Test connection",
      testingObsidian: "Testing…",
      obsidianConnected: "Connected to Obsidian.",
      obsidianConnectionFailed: "Could not connect to Obsidian.",
    ...
    "zh-CN": {
      ...
      obsidianProvider: "Obsidian 同步",
      obsidianApiUrlLabel: "Obsidian API 地址",
      obsidianApiKeyLabel: "Obsidian API 密钥",
      obsidianFolderLabel: "Vault 保存目录",
      obsidianFilenameLabel: "文件名格式",
      obsidianHelp: "请安装并启用 Obsidian Local REST API 插件及其本地 HTTP 服务。",
      testObsidian: "测试连接",
      testingObsidian: "正在测试…",
      obsidianConnected: "已连接到 Obsidian。",
      obsidianConnectionFailed: "无法连接到 Obsidian。",
      saveSettings: "保存设置",
      localRemix: "本地改造",
  ...
  function initialize(root = globalThis) {
    ...
    const obsidianApiUrlInput = doc.getElementById("obsidianApiUrl");
    const obsidianApiKeyInput = doc.getElementById("obsidianApiKey");
    const obsidianFolderInput = doc.getElementById("obsidianFolder");
    const obsidianFilenameInput = doc.getElementById("obsidianFilename");
    const testObsidianBtn = doc.getElementById("testObsidianBtn");
    ...
    async function loadSettings() {
      try {
        ...
        obsidianApiUrlInput.value = settings.obsidian.baseUrl;
        obsidianApiKeyInput.value = settings.obsidian.apiKey;
        obsidianFolderInput.value = settings.obsidian.folder;
        obsidianFilenameInput.value = settings.obsidian.filenameTemplate;
    ...
    async function saveSettings(event) {
      ...
      const settings = settingsApi.normalize({
        ...
        obsidian: {
          baseUrl: obsidianApiUrlInput.value,
          apiKey: obsidianApiKeyInput.value,
          folder: obsidianFolderInput.value,
          filenameTemplate: obsidianFilenameInput.value,
        },
      });
    ...
    testObsidianBtn.addEventListener("click", async () => {
      obsidianStatus.textContent = translate(currentLanguage, "testingObsidian");
      const settings = settingsApi.normalize({
        aiApiKey: aiApiKeyInput.value,
        supadataApiKey: supadataApiKeyInput.value,
        obsidian: {
      ...
      await storage.set({ [settingsApi.STORAGE_KEY]: settings });
      try {
        const result = await root.chrome?.runtime?.sendMessage({ action: "testObsidianConnection" });
        obsidianStatus.textContent = translate(currentLanguage, result?.success ? "obsidianConnected" : "obsidianConnectionFailed");
      } catch (_error) {
        obsidianStatus.textContent = translate(currentLanguage, "obsidianConnectionFailed");
      }
    });

...
async function sendToBackground(message, { idempotent = false } = {}) {
  ...
  for (let attempt = 1; attempt <= BACKGROUND_MAX_ATTEMPTS; attempt += 1) {
    try {
      return await UNIFIED_BILI_BRIDGE.sendMessage(message);
    } catch (error) {
      lastError = error;
      const retryable =
...
async function syncWithActiveTab({ force = false } = {}) {
  ...
  state.analysis = null;
  state.analysisFailures = [];
  // Invalidate the growth chat immediately, even while an older transcript request settles.
  setView("loading");
...
async function loadTranscript({ force = false, lang = "" } = {}) {
  ...
  const promise = (async () => {
    ...
    if (state.bvid !== requestedBvid || state.page !== requestedPage || state.data !== result) return;
    setView("ready");
  })();
...
async function seekTo(seconds) {
  ...
  try {
    await UNIFIED_BILI_BRIDGE.sendTabMessage(state.tabId, {
      action: "seekTo",
...
async function trackPlayback() {
  ...
  try {
    const response = await UNIFIED_BILI_BRIDGE.sendTabMessage(state.tabId, {
...
function setupEventListeners() {
  ...
  el("optionsBtn").addEventListener("click", () => chrome.tabs.create({ url: chrome.runtime.getURL("bilibili-digest/options.html") }));
  ...
  UNIFIED_BILI_BRIDGE.onMessage((message) => {
